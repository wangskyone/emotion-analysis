...
def rec_cfg(cfg, acc):
    with open("record_acc.txt", "a") as f:
        ...
        f.write("LR: {}, Augment Epoch: {}, Train Epoch: {}\n".format(lr, augment_epoch, train_epoch))
...
def main():
    ...
    # ------------------------------------------ Tip-Adapter-F ------------------------------------------
    rec_acc = run_tip_adapter_F(cfg, cache_keys, cache_values, val_features, val_labels, test_features, test_labels, clip_weights, clip_model, train_loader_F)

    # Record the best accuracy and cfg
    rec_cfg(cfg, rec_acc)
